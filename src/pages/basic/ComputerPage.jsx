...
function ComputerPage(props) {
    ...
    const [computerDtail, setComputerDetail] = useState({
        computerId: "",
        company: "",
        cpu: "",
        ram: "",
    ...
    const handleSelectComputerClick = async (computerId) => {
        const data = await requesGetComputer(computerId);
        if(!data) {
            setComputerDetail({
                computerId: "",
                company: "",
    ...
    const requesGetComputer = async (computerId) => {
    ...
    const handleRegisterSubmitClick = async () => {
        try {
            const reponse = await axios.post("http://localhost:8080/api/v1/computer", registerComputer);
    ...
    const handleUpdateSumitClick = async () => {
        await requestUpdate();
        await requestComputerList();
        closeModal();
    }
    ...
    return (
        <div>
            ...
            >
                ...
                `}>
                    <h2>컴퓨터 정보 수정</h2>
                    <input type="text" name='computerId' disabled={true} onChange={handleUpdateInputChange} value={UpdateComputer.computerId} />
                    ...
                    <div>
                        <button onClick={handleUpdateSumitClick}>확인</button>
            ...
            <div css={layout}>
                <h2>목록</h2>
                <p>
                    <input type="text" name='company' onChange={handleSearchInputChange} value={params.company} placeholder='제조사'></input>    
                    <input type="text" name='cpu' onChange={handleSearchInputChange} value={params.cpu} placeholder='CPU'></input>    
            ...
            <div css={layout}>
                <h2>세부정보</h2>
                <ul>
                    <li>ID: {computerDtail.computerId}</li>
                    <li>제조사: {computerDtail.company}</li>
                    <li>CPU: {computerDtail.cpu}</li>
                    <li>RAM: {computerDtail.ram}</li>
                    <li>SSD: {computerDtail.ssd}</li>
                </ul>
            </div>
            <div css={layout}>
                <h2>등록</h2>
                <p>
                    ...
                    value={registerComputer.company}></input>
                </p>
                <p>
                    <label htmlFor="">CPU</label>
                    <input type="text" name='cpu' 
                    onChange={handleRegisterInputChange}
                    value={registerComputer.cpu}></input>
                </p>
                <p>
                    <label htmlFor="">RAM</label>
                    <input type="text" name='ram' 
                    onChange={handleRegisterInputChange}
                    value={registerComputer.ram}></input>
                </p>
                <p>
                    <label htmlFor="">SSD</label>
                    <input type="text" name='ssd'  
                    onChange={handleRegisterInputChange}
                    value={registerComputer.ssd}></input>
                </p>
                <p>
